...
from model_style.generation_style import Generation_style
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        self.c_attn = Conv1D_style(n_state * 3, nx, Num_cluster)
        self.c_proj = Conv1D_style(n_state, nx, Num_cluster)
    ...
    def forward(
        self, x, layer_past=None, attention_mask=None, head_mask=None, use_cache=False, output_attentions=False, cluster=None
    ):
        x = self.c_attn(x, cluster)
...
class MLP(nn.Module):
    def __init__(self, n_state, config):  # in MLP: n_state=3072 (4 * n_embd)
        super().__init__()
        nx = config.n_embd
        self.c_fc = Conv1D_style(n_state, nx, Num_cluster)
        self.c_proj = Conv1D_style(nx, n_state, Num_cluster)
        self.act = ACT2FN[config.activation_function]
        self.dropout = nn.Dropout(config.resid_pdrop)

    def forward(self, x, cluster):
        h = self.act(self.c_fc(x, cluster))
        h2 = self.c_proj(h, cluster)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        x = x + a
        m = self.mlp(self.ln_2(x), cluster)
        x = x + m
...
class GPT2PreTrainedModel(PreTrainedModel, Generation_style):
...
class GPT2LMHeadModel_style(GPT2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
